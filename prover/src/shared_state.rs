...
impl SharedState {
    ...
    //for benchmark_test
    pub async fn circuit_check(&self)->Result<Proofs,String>{

        let witness:CircuitWitness = CircuitWitness::dummy(63000).unwrap();
        let mut task_options_copy:ProofRequestOptions=ProofRequestOptions::default();
        task_options_copy.circuit="pi".to_string();
        task_options_copy.param="./testnet/".to_string();
